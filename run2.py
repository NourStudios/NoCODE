...
def interactive_test_tk(models, test_image_path, grid_size, reshape_size):
    ...
    creativeness_scale.set(0.1)  # default noise
    ...
    def update_images(command):
        ...
        # Show only the reconstructed image
    ...
    def auto_press():
        while True:
            time.sleep(0.05)
            if key_state['w']:
                update_images('w')  # Jump while 'w' is held
            else:
                update_images('s')  # Default to fall
    ...
    def on_key_press(event):
        ...
        if key == 'q':
            ...
            key_state['w'] = True  # Start holding 'w'

    def on_key_release(event):
        key = event.char.lower()
        if key == 'w':
            key_state['w'] = False  # Stop holding 'w'
